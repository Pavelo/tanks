...
struct _tank
{
	...
	float v[3];               //vettore per la velocit‡
...
// Controlla se è avvenuta una collisione con i confini della mappa
...
// Controlla se è avvenuta una collisione con qualche ostacolo statico della mappa
...
// Controlla se è avvenuta una collisione con qualche powerup
...
// Controlla se è avvenuta una collisione tra i carri armati
...
void shoot(tank *tankT)
{
	...
	float translateB = (tankT->userTurret.tankCannon.pos[2] - tankT->userTurret.tankCannon.length);
	b->bullet.pos[0] = translateB*sin((tankT->rot+tankT->userTurret.rot)*M_PI/180.0f);
	b->bullet.pos[1] = 0.0f;
	b->bullet.pos[2] = translateB*cos((tankT->rot+tankT->userTurret.rot)*M_PI/180.0f);
	translateB = tankT->userTurret.tankCannon.length;
	b->bullet.pos[0] = (-translateB*cos(tankT->userTurret.tankCannon.rot*M_PI/180.0f)-translateB*0.5f)*sin((tankT->rot+tankT->userTurret.rot)*M_PI/180.0f);
	b->bullet.pos[1] = translateB*sin(tankT->userTurret.tankCannon.rot*M_PI/180.0f);
	b->bullet.pos[2] = (-translateB*cos(tankT->userTurret.tankCannon.rot*M_PI/180.0f)-translateB*0.5f)*cos((tankT->rot+tankT->userTurret.rot)*M_PI/180.0f);
	//*cos(tankT->userTurret.tankCannon.rot*M_PI/180.0f)
	float cannonSpeed = 35.0f;
    //la velocit‡ di sparo viene separata in componente verticale e orizzontale rispetto alla rotazione del cannone
...
//controlla se un angolo di rotazione Ë all'interno di un certo step
...
void attackEnemy(tank *tankT)
{
	...
	{    
		//se non c'Ë collisione nella direzione verso cui dovrei ruotare
		if(checkCollision(tankT,degrees(angle*XSign))==0)
		{                                      
			float stepR = 2.0f;
			//se la rotazione Ë entro lo step di rotazione del nemico
			...
			//altrimenti se la rotazione Ë al di fuori dello step di rotazione del nemico
			else
			{
		...
		{
			//entra in modalit‡ collision avoidance
			tankT->collision = 10;
			float stepC = 30.0f;
			int i;
			for(i=1;i<5;i++)
			{
				//se non c'Ë collisione all'angolo di collisione + 10*i
...
{
	//se non Ë stata rilevata una collisione
	//PLAN ACTION
	//se non hai abbastanza vita o munizioni
	//scappa
	//altrimenti se il nemico Ë in visuale
	//attacca
	//altrimenti
	//movimento casuale
	//se Ë stata rilevata collisione
	//EVITA COLLISIONE
	...
	if(tankT->rot<0.0f)
		TSign = -1.0f;
	float angle = acos(distanceZ/distance);// * XSign;
										   //se non Ë stata rilevata una collisione in precedenza
    ...
    //se in precedenza hai rilevato una collisione (modalit‡ collision avoidance)
    else
    {
    	...
    	for(p=tanks[i].bulletRoot; p!=NULL; p=p->next)
        {
			...
			if (p->lifetime < LIFETIME)
			{
				...
				glPushMatrix();
					...
					glTranslatef(tanks[i].pos[0],tanks[i].pos[1]+tanks[i].userTurret.pos[1],tanks[i].pos[2]);
...
//		tanks[i].userTurret.rot = fmodf(tanks[i].userTurret.rot, 180.0f);
    	...
    	tanks[i].v[0] += tanks[i].a[0] * deltaT; //velocit‡ finale
    	tanks[i].pos[2] += tanks[i].v[2]*deltaT + tanks[i].a[2]*deltaT*deltaT*0.5f;
    	tanks[i].v[2] += tanks[i].a[2] * deltaT; //velocit‡ finale
